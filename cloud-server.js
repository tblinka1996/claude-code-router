...
// This file starts the server directly for cloud platforms like Render
...
const startServer = async () => {
  ...
  // We need to directly import the run function from the index module
  // Since the CLI wrapper might interfere with cloud deployment
  ...
  try {
    // Try to import the run function from the built index
    const indexModule = require('./dist/cli.js');
    
    // Look for the run function in the module
    if (indexModule.run) {
      await indexModule.run({ port });
    } else {
      // Fallback: try to start the service using the CLI approach
      console.log('Attempting to start service...');
      
      // Simulate the start command
      process.argv = ['node', 'cloud-server.js', 'start'];
      require('./dist/cli.js');
    }
  } catch (error) {
